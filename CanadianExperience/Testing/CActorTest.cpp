...
namespace Testing
{
	...
	public:
		...
		{
			CActor actor(L"lolist");
			Assert::IsTrue(actor.GetName() == L"lolist");
		}

		TEST_METHOD(TestEnabled)
		{
			CActor actor(L"TestActor");
		...
		TEST_METHOD(TestPosition)
		{
			...
			actor.SetPosition(Gdiplus::Point(20, 30));
			Assert::AreEqual(20, actor.GetPosition().X);
			Assert::AreEqual(30, actor.GetPosition().Y);
		}
