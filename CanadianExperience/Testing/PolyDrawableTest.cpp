...
namespace Testing
{
	...
	public:
		...
		bool CompareColor(Gdiplus::Color a, Gdiplus::Color b)
		{
			return (a.GetA() == b.GetA() && a.GetR() == b.GetR() && a.GetG() == b.GetG() && a.GetB() == b.GetB());
		}
		...
		{
			CPolyDrawable poly(L"test");
			Assert::IsTrue(poly.GetName() == L"test");
		}

		TEST_METHOD(TestColor)
		{
			CPolyDrawable poly(L"test");
			Assert::IsTrue(CompareColor(poly.GetColor(), Gdiplus::Color::Black));
			poly.SetColor(Gdiplus::Color::Blue);
			Assert::IsTrue(CompareColor(poly.GetColor(), Gdiplus::Color::Blue));
		}
