...
public:
	~CTimeline();
	CTimeline();
	CTimeline(CTimeline& time) = delete; ///< copy constructor disabled
 ...
 */
     ...
     */
    double GetDuration() const { return mNumFrames / mFrameRate; }

    virtual void AddChannel(CAnimChannel* c);

    virtual double GetTime() { return mCurrentTime; };
private:
	int mNumFrames = 300;
	int mFrameRate = 30;
